...
class SignedReportsDownloadWorker(var context: Context, workerParams: WorkerParameters) :
    ...
    //In this class TAG is showing an error because it can be only 23 characters long

    companion object {
        private const val TAG = "SignedReportsDownloadWorker"
    ...
    override fun doWork(): Result {
        ...
        var fetchSinceTime = context.getSharedPreferences(
            context.getString(R.string.preference_file_key),//check this here
        ...
        val task =
            FirebaseFirestore.getInstance().collection(FirestoreConstants.COLLECTION_SIGNED_REPORTS)
                ...
                .get()
                .continueWith(
                    CoronaControlDatabase.databaseWriteExecutor,
                    Continuation<QuerySnapshot, Result> { task ->
                        ...
                        if (queryDocumentSnapshots != null) {
                            ...
                            result = try {
                                ...
                                markLocalTemporaryContactNumbers(queryDocumentSnapshots.documentChanges, true)
        ...
        documentChanges.forEach { documentChange ->
            ...
            val temporaryContactKeyBytes =
                (data[FirestoreConstants.FIELD_TEMPORARY_CONTACT_KEY_BYTES] as? Blob)?.toBytes()
                    ?: return@forEach
            val endIndex = (data[FirestoreConstants.FIELD_END_INDEX] as? Long)?.toShort() ?: return@forEach
            val memoData = (data[FirestoreConstants.FIELD_MEMO_DATA] as? Blob)?.toBytes() ?: return@forEach
            val memoType = (data[FirestoreConstants.FIELD_MEMO_TYPE] as? Long)?.toShort() ?: return@forEach
            val reportVerificationPublicKeyBytes =
                (data[FirestoreConstants.FIELD_REPORT_VERIFICATION_PUBLIC_KEY_BYTES] as? Blob)?.toBytes()
                    ?: return@forEach
            val signatureBytes =
                (data[FirestoreConstants.FIELD_SIGNATURE_BYTES] as? Blob)?.toBytes() ?: return@forEach
            val startIndex = (data[FirestoreConstants.FIELD_START_INDEX] as? Long)?.toShort() ?: return@forEach
